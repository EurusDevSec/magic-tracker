...
import {
  ChevronLeft, Save, Loader2, Plus, X, ImageIcon, Upload
} from 'lucide-react'
...
export default function NewMeetingPage() {
  ...
  useEffect(() => {
    setMeetingDate(new Date().toLocaleDateString('en-CA'))
  }, [])
  ...
  const handleImageUpload = async (files: FileList | null) => {
    ...
    try {
      ...
      setErrorMsg('Lỗi khi xử lý ảnh. Vui lòng thử lại.')
    } finally {
      setCompressing(false)
    }
  }

  const removeImage = (i: number) => setAttachments(prev => prev.filter((_, idx) => idx !== i))

  const handleSubmit = async () => {
    if (!user) return
    setErrorMsg('')
    if (!title.trim()) { setErrorMsg('Vui lòng nhập tiêu đề buổi họp.'); return }
    if (!summary.trim()) { setErrorMsg('Vui lòng nhập tóm tắt nội dung buổi họp.'); return }
    ...
    try {
      const { error } = await supabase.from('boss_meetings').insert([payload])
      if (error) throw error
      router.push('/meetings')
    } catch (err: any) {
      setErrorMsg(err.message || 'Lỗi khi lưu buổi họp.')
      setSubmitting(false)
    }
  }
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto w-full">
        <div className="max-w-3xl mx-auto w-full space-y-6">
          ...
          <div className="flex items-center gap-3 border-b border-white/5 pb-4">
            <Link href="/meetings" className="text-slate-400 hover:text-white transition-colors cursor-pointer">
              <ChevronLeft className="h-6 w-6" />
            </Link>
            <div>
              <h1 className="text-2xl font-extrabold text-white tracking-tight">Ghi Nhận Buổi Họp</h1>
              <p className="text-slate-400 text-sm mt-0.5">Lưu lại nội dung để toàn đội cùng nắm được</p>
            </div>
          </div>

          {errorMsg && (
            <div className="p-3.5 rounded-xl bg-rose-500/10 border border-rose-500/25 text-rose-400 text-sm font-semibold">
              {errorMsg}
            </div>
          )}
          ...
          <div className="space-y-5">
            ...
            <div className="glass-card rounded-2xl p-5 space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1.5">Tiêu đề buổi họp <span className="text-rose-400">*</span></label>
                <input
                  type="text"
                  value={title}
                  onChange={e => setTitle(e.target.value)}
                  placeholder="VD: Buổi Họp Định Hướng — Hành Trình Phát Triển Bản Thân"
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1.5">Giờ bắt đầu</label>
            ...
            <div className="glass-card rounded-2xl p-5 space-y-3">
              <h3 className="text-xs font-bold uppercase tracking-wider text-violet-400">Chủ đề thảo luận</h3>
              <p className="text-xs text-slate-500">Chọn các chủ đề phù hợp với nội dung buổi họp</p>
                  ...
                  return (
                    <button
                      ...
                      className={`text-xs font-semibold px-3 py-1.5 rounded-full border transition-all duration-200 cursor-pointer ${
                        isSelected
                          ? (TAG_COLORS[tag] || 'bg-violet-500/20 text-violet-300 border-violet-500/40')
                          : 'bg-slate-900/30 text-slate-400 border-white/10 hover:border-white/20'
                      }`}
                    >
                      {isSelected && '✓ '}{tag}
                    </button>
            ...
            {/* Tóm tắt */}
            <div className="glass-card rounded-2xl p-5 space-y-3">
              <h3 className="text-xs font-bold uppercase tracking-wider text-violet-400">Tóm tắt nội dung <span className="text-rose-400">*</span></h3>
              <textarea
                value={summary}
                onChange={e => setSummary(e.target.value)}
                placeholder="Tóm tắt những gì được thảo luận trong buổi họp hôm nay..."
                rows={5}
            ...
            {/* Lời khuyên vàng */}
            <div className="glass-card rounded-2xl p-5 space-y-3 border-amber-500/10">
              <div className="flex items-center gap-2">
                <h3 className="text-xs font-bold uppercase tracking-wider text-amber-400">Lời khuyên / Định hướng từ Mentor</h3>
                <span className="text-xs text-slate-500">(hỗ trợ Markdown)</span>
              </div>
              <textarea
                value={decisions}
                onChange={e => setDecisions(e.target.value)}
                placeholder={'> "Câu quote của sếp..."\n> — Tên mentor\n\n**Định hướng:**\n- Điểm 1\n- Điểm 2'}
                rows={6}
            ...
            {/* Thử thách */}
            <div className="glass-card rounded-2xl p-5 space-y-3">
              <h3 className="text-xs font-bold uppercase tracking-wider text-violet-400">Thử thách / Hành động được đề cập</h3>
            ...
            <div className="glass-card rounded-2xl p-5 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-slate-500">{attachments.length}/{MAX_IMAGES} ảnh</span>
              </div>
              ...
              {attachments.length < MAX_IMAGES && (
                <div
                  ...
                  className="border-2 border-dashed border-white/10 hover:border-violet-500/40 rounded-xl p-6 flex flex-col items-center gap-2 cursor-pointer transition-all duration-200 group"
                >
                  {compressing ? (
                    <Loader2 className="h-6 w-6 animate-spin text-violet-400" />
                  ) : (
                    <Upload className="h-6 w-6 text-slate-500 group-hover:text-violet-400 transition-colors" />
                  )}
                  <p className="text-xs text-slate-400 group-hover:text-slate-300 transition-colors text-center">
                    {compressing ? 'Đang nén ảnh...' : 'Click hoặc kéo thả ảnh vào đây'}
                  </p>
                  <p className="text-[10px] text-slate-600">Tối đa {MAX_IMAGES} ảnh • Tự động nén về 800px JPEG</p>
                  <input
                    ref={fileInputRef}
                    type="file"
                    accept="image/*"
            ...
            <div className="flex justify-end gap-3 pt-2 pb-8">
              ...
              >
                ...
                {submitting ? 'Đang lưu...' : 'Lưu buổi họp'}
              </button>
            </div>
          </div>
        </div>
